...
เรียงตามลำดับที่ปลดล็อกงานได้มากที่สุด

1. **WP-00C behavior baseline** — ตัวปลดล็อก Gate 1D ตัวจริง งานใหญ่และต้องมีฝั่งธุรกิจ
   ร่วมนิยามว่าอะไรคือผลลัพธ์ที่ถูก
2. **ขยาย smoke ไปเส้นทางที่ยังไม่แตะ** — Excel import (ใช้ temp table 4 ใบ + PHPExcel),
   reset password, ส่งอีเมล, report ที่ข้อมูลเยอะ งานเล็กกว่าและน่าจะเจอปัญหาการย้าย DB เพิ่ม
3. **แก้ controller `Error`** — ต้องตัดสินก่อนว่าจะแก้ที่รีโป CI3 (ต้องเปิดให้เขียนได้
   ซึ่งขัดกฎปัจจุบัน) หรือรอไปแก้ตอนพอร์ตเป็น CI4
4. **WP-00D rotate credential** — งานฝั่ง security ทำแยกได้เลย ไม่ต้องรอ Gate
