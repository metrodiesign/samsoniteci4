...
function renderReport(
    ...
    $out[] = '| `FN-BLK-005` Retirement approval | static no-caller ไม่พิสูจน์ production no-use — 247 จุดที่ไฟล์หายปิดแล้วด้วย deletion evidence + owner decision 2026-08-17 (rollback ทำได้จาก production host เท่านั้น) | คงเปิดสำหรับ retirement ของ live points ในอนาคต: traffic/coverage/external-consumer=0 + business owner approval + rollback |';
...
function retiredSection(array $retired, array $retiredByOrigin): string
{
    ...
    $out[] = '';
    $out[] = 'Business owner approval: project owner ตัดสินเมื่อ 2026-08-17 ให้ปิดทั้ง 247 จุดด้วยหลักฐาน no-caller (รวม 83 จุดที่ v1 เคยเป็น `MIGRATE` ซึ่งถามแยกและอนุมัติแยก) — บันทึกการตัดสินอยู่ใน PR ของ commit นี้ นี่คือ approval ที่ AC field `Retirement` และ `FN-BLK-005` เรียกหาสำหรับจุดกลุ่มนี้.';
    $out[] = '';
