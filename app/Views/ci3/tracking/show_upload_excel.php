<div class="content-wrapper">
    ...
    <div class="content-form">
      <section class="content-header">
        <h1>
          <i class="fa fa-link"></i> Data Upload file Management
        </h1>
      </section>
      <section class="content">
  <form action="<?php echo base_url() ?>ExcelConfirm" method="POST" id="sendorderUpdate">
          <div class="row">
               <div class="col-xs-12">
                 <div class="box">
                  ...
                  <div class="box-body table-responsive no-padding">
                     <table class="table table-hover" >
                      ...
                      {
                        //var_dump($sheet_data);
                        $xi=1;$result_status_s=0;$action_status="";$trackID="";$countorderID=0;$data_err_orderid="";$si=0;$temp_number_cmg="";
                          ...
                          foreach($sheet_data as $record)
                          {
                            ...
                            if(empty($trackID) or $trackID=="")
                            {
                              ...
                              if($low_temp_orderIDShow==1){
                                $data_err_orderid=" (เบอร์โทรไม่ตรงกับระบบ) ";
                              }else{
                                $data_err_orderid=" (เล่มที่/เลขที่ ไม่มีในระบบ) ";
                              }
                            ...
                            }else
                            {
                              if($countorderID>1){
                                ...
                                $data_class_error=1;
                                $data_err_orderid=" (เล่มที่/เลขที่ มีมากกว่า 1) ";
                              }else{
                                if($action_status >3){
                                  $class_error="";
                                }else if($action_status ==1){
                                  $class_error="class='newstatus-table'";
                                  $data_err_orderid=" (Status New) ";
                                }else{
                            ...
                            if($waranty_cmg=="" or $temp_Status=="" or $temp_Update=="" or $temp_recripUpdate==""
                            or $class_error <>"" or strlen($temp_Update)<10  or strlen($temp_recripUpdate)<10 or substr($temp_Update,-4)>$yyyy or substr($temp_recripUpdate,-4)>$yyyy or $temp_number_cmg=="")
                            {
                              $si++;
                              $dis_style='';
                            }else{
                       ...
                       $xi++;
                            ...
                            if($si==0){
                              echo "ข้อมูลถูกต้อง กรุณากด Comfirm เพื่ออัพโหลดสถานะ";
                              ?>
                              ...
                              echo "กรุณาตรวจสอบข้อมูลเล่มที่/เลขที่, เบอร์โทรศัพท์ ,วันที่ และ เลขที่ CMG ให้ถูกต้อง";
                              ?>
                              <script type="text/javascript">
                              $(document).ready(function(){
                                  //$('#submit_list').cc('btn btn-primary disabled');
